...
function UserUploadImage() {
    const {url, user, setupload, setpicup} = useContext(userContext)
  ...
  const handleSubmit = async(event) => {
    ...
    setloading(true)
    try {
        const res = await fetch(`${url}/user/imageupload`,{
            method : "POST",
            credentials : "include",
            headers : {
                "Content-Type" : "application/json"
            },
            body : JSON.stringify({id : user?._id, image : profileImage})
        })
  ...
  return (
    <div className="p-3 mt-3 max-w-md mx-auto bg-white shadow-md rounded-lg relative">
